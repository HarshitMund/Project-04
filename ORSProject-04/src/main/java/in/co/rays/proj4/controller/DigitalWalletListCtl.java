...
@WebServlet(name = "DigitalWalletListCtl", urlPatterns = { "/ctl/DigitalWalletListCtl" })
public class DigitalWalletListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<DigitalWalletBean> list = model.search(bean, pageNo, pageSize);
			List<DigitalWalletBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					DigitalWalletBean deletebean = new DigitalWalletBean();
					for (String id : ids) {
						deletebean.setId(DataUtility.getInt(id));
						model.delete(deletebean);
						ServletUtility.setSuccessMessage("Digital Wallet record deleted successfully", request);
					}
				} else {
	...
	protected String getView() {
		return ORSView.DIGITAL_WALLET_LIST_VIEW;
	}
}