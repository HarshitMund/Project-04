...
@WebServlet(name = "CollegeSystemListCtl", urlPatterns = { "/ctl/CollegeSystemListCtl" })
public class CollegeSystemListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<CollegeSystemBean> list = model.search(bean, pageNo, pageSize);
			List<CollegeSystemBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					CollegeSystemBean deletebean = new CollegeSystemBean();
					for (String id : ids) {
	...
	protected String getView() {
		return ORSView.COLLEGE_SYSTEM_LIST_VIEW;
	}
}