...
public class AppointmentListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<AppointmentBean> list = model.search(bean, pageNo, pageSize);
			List<AppointmentBean> next = model.search(bean, pageNo + 1, pageSize);
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					for (String id : ids) {
						...
						ServletUtility.setSuccessMessage("Appointment record deleted successfully", request);
					}
				} else {
